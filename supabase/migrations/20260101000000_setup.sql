...
BEGIN
  ...
  -- Se a venda não tiver usuário (IS NULL) ou for de outro usuário, só ADMIN/MANAGER pode cancelar
  IF (v_sale.user_id IS NULL OR v_sale.user_id != auth.uid()) AND public.current_user_role() NOT IN ('ADMIN', 'MANAGER') THEN
