...
-- Reforça autorização por papel, descontos, idempotência e estado da venda.
...
BEGIN
  ...
  IF NOT public.has_store_access(p_store_id)
     OR public.get_user_store_role(p_store_id) NOT IN ('ADMIN', 'MANAGER', 'CASHIER') THEN
  ...
  IF p_idempotency_key IS NOT NULL AND btrim(p_idempotency_key) <> '' THEN
    ...
    FOR SHARE;
    IF FOUND THEN
      SELECT total, sale_number INTO v_final_total, v_sale_number
  ...
  v_normalized_method := CASE
    ...
    WHEN UPPER(COALESCE(p_payment_method, '')) LIKE '%CASH%' OR UPPER(COALESCE(p_payment_method, '')) LIKE '%DINHEIRO%' THEN 'CASH'
    ELSE 'CASH'
  END;

  IF v_resolved_customer_id IS NULL AND p_customer_cpf IS NOT NULL AND p_customer_cpf NOT IN ('', 'Não informado') THEN
    SELECT id INTO v_resolved_customer_id FROM public.customers WHERE cpf = trim(p_customer_cpf) LIMIT 1;
  ...
  FOR v_item IN SELECT * FROM jsonb_to_recordset(p_items) AS (variant_id UUID, quantity INT, unit_price NUMERIC, product_name TEXT, variant_description TEXT) LOOP
    ...
    SELECT v_sale_id, pv.product_id, v_item.variant_id, COALESCE(v_item.product_name, p.name), COALESCE(v_item.variant_description, pv.size || ' / ' || pv.color), v_item.quantity, v_real_price, v_item_total
    FROM public.product_variants pv JOIN public.products p ON p.id = pv.product_id
